...
| `phase()`, `log()`, structured events, pretty/JSON/JSONL output, and event-derived timing summaries | `verified` | `static`, `fixture-provider` | `tests/worker/sandbox.test.ts` — `proxies agent, phases, logs and every console level over IPC`; `tests/core/timing.test.ts` — run, attempt, parallel phase, call, and resume timing; `tests/cli/run.test.ts` — JSON terminal timing and JSONL event output; `tests/cli/resume.test.ts` — stored-event timing through resume and `runs show` | Output modes, event ordering, old-run derivation, parallel call time, and resume idle time are asserted. Timing is provider-neutral and does not claim provider-internal latency boundaries. |
...
| Provider-neutral fast model alias | `verified` | `static` | `tests/config/model-map.test.ts` — `freezes defaults for codex/haiku` and `freezes defaults for claude/haiku` | The same Workflow `haiku` request resolves to native Haiku on Claude and the configured `gpt-5.6-luna` low-effort fast tier on Codex. This is routing evidence, not a live Luna quality or latency claim. |
...
| Codex stream success, failure, schema, usage, and safe transient retry classification | `verified` | `real-provider`, `static` | No-tools acceptance, `tests/providers/codex.test.ts` — `consumes a complete event stream and preserves public observations`, `marks a transient pre-output turn failure as safely recoverable`, `does not recover a transient failure after substantive provider output` — and `tests/runtime/engine.test.ts` — `retries safe transient provider failures inside one logical call`, `writes one terminal failure after safe retries are exhausted`; see [real Codex acceptance record](../implementation/260729-real-codex-acceptance.md). | Static adapter tests cover the public `codex exec --json` shapes and bounded runtime retry. The real smoke covers the success path and exact version; live failure timing remains oracle-gated. |
